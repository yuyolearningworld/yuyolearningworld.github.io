...
fetch("../@misc/subjects.json", {method: 'GET'})
.then(subjectLesson => subjectLesson.json())
.then(subjectLesson => {
	...
	for (var i=0; i<subjects.length; i++) {
		...
		lessons = Object.keys(subjectLesson[subjects[i]])
		for (var l=0; l<lessons.length; l++) {
			document.write(`<p class="lessontext" id="subject`+ Number(i+1) +`_lesson`+ Number(l+1) +`">` + SUBJECTsubjectLessonESSONS[subjects[i]][lessons[l]] + `
